...
import {
  ...
  Shield, Users, Hash, Sparkles, ChevronRight, RotateCcw, Play,
} from "lucide-react";

/* ============================================================
   WordScout11Plus — single-file working prototype
   - All data is in-memory (React state) with sample sets.
   - Auth is mocked. Replace with Supabase/Firebase when wiring a backend.
   - 8 game modes, dashboard, admin, dark/light mode.
   ============================================================ */
...
// Game palettes — kept within the brand teal/gold family, with subtle shifts
// so each game still feels distinct without going off-brand.
const GAME_THEMES = {
  flashcards: { name: "Flashcards", accent: "#0d9488", icon: BookOpen },   // teal
  fillblank: { name: "Fill the Blank", accent: "#115e59", icon: Edit },     // deep teal
  spelling: { name: "Spelling Test", accent: "#d97706", icon: Volume2 },    // brand gold
  match: { name: "Match-Up Pairs", accent: "#0f766e", icon: Sparkles },     // brand teal
  kahoot: { name: "Quiz Rush", accent: "#b45309", icon: Trophy },           // deep amber
  goldhunt: { name: "Gold Hunt", accent: "#d97706", icon: Star },           // brand gold
  hangman: { name: "Hangman", accent: "#14b8a6", icon: User },              // bright teal
  wordle: { name: "Word Guess", accent: "#0f766e", icon: Hash },            // brand teal
};
...
export default function App() {
  ...
  const [view, setView] = useState({ name: "landing" }); // {name, ...params}
  const [progress, setProgress] = useState({}); // setId -> { mode: stars }
  const [streak] = useState(5);
  const [showSettings, setShowSettings] = useState(false);

  const bg = dark ? "#0f1729" : "#f4f7f5";
  const surface = dark ? "#1a2336" : "#ffffff";
  const text = dark ? "#e7ecf3" : "#1a2336";
  const muted = dark ? "#8fa0b8" : "#5f6b7c";
  const border = dark ? "#2a3550" : "#e2e8e4";
  const brand = "#0f766e"; // deep teal
  const gold = "#d97706";

  const t = { bg, surface, text, muted, border, brand, gold, dark };
  ...
  const Nav = () => (
    <div style={{
      ...
      <div onClick={() => go(user ? { name: "dashboard" } : { name: "landing" })}
        ...
        <span style={{ fontWeight: 800, fontSize: 19, color: text, letterSpacing: -0.5 }}>
          WordScout<span style={{ color: gold }}>11Plus</span>
        </span>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <button onClick={() => setShowSettings((s) => !s)} title="Settings"
          style={iconBtn(t)}><Settings size={18} /></button>
        {user ? (
          ...
          <button onClick={() => go({ name: "login" })}
            style={{ ...pill(brand), marginLeft: 4 }}>Sign in</button>
        )}
      </div>

      {showSettings && (
        <div style={{
          position: "absolute", right: 20, top: 64, background: surface,
          border: `1px solid ${border}`, borderRadius: 14, padding: 14, width: 220,
          boxShadow: "0 10px 40px rgba(0,0,0,0.18)", zIndex: 60,
        }}>
          <div style={{ fontWeight: 700, color: text, marginBottom: 10 }}>Settings</div>
          <button onClick={() => setDark((d) => !d)}
            style={{
              display: "flex", alignItems: "center", gap: 10, width: "100%",
              padding: "10px 12px", borderRadius: 10, border: `1px solid ${border}`,
              background: bg, color: text, cursor: "pointer", fontSize: 14,
            }}>
...
const NavLink = ({ active, onClick, icon: Icon, label, t }) => (
  <button onClick={onClick} style={{
    ...
    background: active ? t.brand : "transparent", color: active ? "#fff" : t.muted,
  }}>
    <Icon size={16} /> <span style={{ display: window.innerWidth < 640 ? "none" : "inline" }}>{label}</span>
  </button>
);
const card = (t) => ({
  background: t.surface, border: `1px solid ${t.border}`, borderRadius: 18, padding: 20,
});
  ...
  return (
    <div style={{ textAlign: "center", padding: "60px 0 80px" }}>
      ...
      <h1 style={{ fontSize: 46, margin: "0 0 12px", color: t.text, letterSpacing: -1 }}>
        Make 11+ vocabulary feel like an <span style={{ color: t.gold }}>adventure</span>
      </h1>
      ...
      <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
        ...
        <button onClick={onJoin} style={{
          padding: "14px 28px", borderRadius: 999, border: `2px solid ${t.border}`,
          background: t.surface, color: t.text, fontWeight: 800, fontSize: 16, cursor: "pointer",
    ...
    <div style={{ maxWidth: 400, margin: "50px auto" }}>
      <div style={card(t)}>
        ...
        <p style={{ textAlign: "center", color: t.muted, fontSize: 13, marginTop: 16 }}>
          ...
          <span onClick={switchMode} style={{ color: t.brand, fontWeight: 800, cursor: "pointer" }}>
...
function JoinByCode({ t, sets, onPlay }) {
  ...
  const join = () => {
    // demo: any code routes to first set, flashcards
  ...
  return (
    <div style={{ padding: "28px 0 60px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: t.gold + "22", padding: "8px 14px", borderRadius: 999 }}>
          <Flame size={18} color={t.gold} />
          <span style={{ fontWeight: 800, color: t.gold }}>{streak} day streak!</span>
        </div>
      </div>

      {thisWeek && (
        <div onClick={() => onOpen(thisWeek.id)} style={{
          ...card(t), marginTop: 22, background: t.brand, color: "#fff", cursor: "pointer",
          border: "none", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12,
        }}>
          <div>
            <div style={{ fontSize: 12, fontWeight: 800, opacity: 0.85, textTransform: "uppercase", letterSpacing: 1 }}>This week's set</div>
            <div style={{ fontSize: 26, fontWeight: 900, fontFamily: "Fredoka" }}>{thisWeek.title}</div>
            <div style={{ opacity: 0.9 }}>{thisWeek.items.length} words · {thisWeek.topic}</div>
          </div>
        ...
        {sets.map((s) => (
          <div key={s.id} onClick={() => onOpen(s.id)} style={{ ...card(t), cursor: "pointer" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 12, fontWeight: 800, color: t.brand, background: t.brand + "1a", padding: "4px 10px", borderRadius: 999 }}>
                Week {s.week}
              </span>
            ...
            <div style={{ display: "flex", gap: 3 }}>
              {[...Array(8)].map((_, i) => (
                <Star key={i} size={16} fill={i < Math.min(8, starsFor(s.id)) ? t.gold : "none"} color={i < Math.min(8, starsFor(s.id)) ? t.gold : t.border} />
              ))}
            </div>
          </div>
        ))}
    ...
    <div style={{ padding: "24px 0 60px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(180px,1fr))", gap: 14 }}>
        {Object.entries(GAME_THEMES).map(([key, g]) => (
          <div key={key} onClick={() => onPlay(key)} style={{
            ...card(t), cursor: "pointer", textAlign: "center", borderTop: `4px solid ${g.accent}`,
          }}>
            ...
            <div style={{ display: "flex", gap: 2, justifyContent: "center", marginTop: 6 }}>
              {[1, 2, 3].map((i) => (
                <Star key={i} size={13} fill={i <= (progress[key] || 0) ? t.gold : "none"} color={i <= (progress[key] || 0) ? t.gold : t.border} />
              ))}
            </div>
          </div>
        ))}
...
function GameHost({ t, set, mode, onExit, onFinish }) {
  ...
  const [done, setDone] = useState(null); // {stars, score, total, missed}
  if (!set) return null;
  ...
  return (
    <div style={{ padding: "20px 0 60px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontWeight: 800, color: g.accent }}>
          <g.icon size={20} /> {g.name}
        </div>
      </div>
  ...
  return (
    <div style={{ ...card(t), textAlign: "center", maxWidth: 460, margin: "30px auto", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", gap: 6, justifyContent: "center", margin: "12px 0" }}>
        {[1, 2, 3].map((i) => (
          <Star key={i} size={42} fill={i <= result.stars ? t.gold : "none"} color={i <= result.stars ? t.gold : t.border} className="pop" />
        ))}
      </div>
...
function Confetti({ accent }) {
  const colors = [accent, "#0f766e", "#d97706", "#14b8a6", "#b45309"];
  return (
...
function FillBlank({ t, set, accent, onFinish }) {
  ...
  const next = () => {
    if (i + 1 >= set.items.length) {
      const final = score;
      onFinish({ stars: stars(final, set.items.length), score: final, total: set.items.length });
    ...
    <div style={{ maxWidth: 520, margin: "0 auto" }}>
      ...
      <div style={{ ...card(t), margin: "16px 0", borderTop: `5px solid ${accent}` }}>
        ...
        <input autoFocus value={val} disabled={!!feedback} onChange={(e) => setVal(e.target.value)}
          ...
          style={{ width: "100%", padding: 14, fontSize: 18, borderRadius: 12, border: `2px solid ${feedback ? (feedback.correct ? "#22c55e" : "#ef4444") : t.border}`, background: t.bg, color: t.text, outline: "none" }} />
        {feedback && (
          <div style={{ marginTop: 12, color: feedback.correct ? "#22c55e" : "#ef4444", fontWeight: 800 }}>
...
function Spelling({ t, set, accent, onFinish }) {
  ...
  const next = () => {
    if (i + 1 >= set.items.length) {
      onFinish({ stars: stars(score, set.items.length), score, total: set.items.length, missed });
    } else { setI(i + 1); setVal(""); setFeedback(null); }
  };
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", textAlign: "center" }}>
      ...
      <div style={{ ...card(t), margin: "16px 0", borderTop: `5px solid ${accent}` }}>
        ...
        <input autoFocus value={val} disabled={!!feedback} onChange={(e) => setVal(e.target.value)}
          ...
          style={{ width: "100%", textAlign: "center", padding: 14, fontSize: 22, fontWeight: 800, borderRadius: 12, border: `2px solid ${feedback ? (feedback.correct ? "#22c55e" : "#ef4444") : t.border}`, background: t.bg, color: t.text, outline: "none" }} />
        {feedback && <div style={{ marginTop: 12, color: feedback.correct ? "#22c55e" : "#ef4444", fontWeight: 800 }}>{feedback.correct ? "Spot on!" : `Correct spelling: ${it.term}`}</div>}
...
function MatchUp({ t, set, accent, onFinish }) {
  ...
  const click = (card) => {
    if (flipped.length === 2 || flipped.find((c) => c.id === card.id) || matched.includes(card.pair)) return;
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 10 }}>
        {deck.map((card) => {
          const isUp = flipped.find((c) => c.id === card.id) || matched.includes(card.pair);
          return (
            <div key={card.id} onClick={() => click(card)} style={{
              minHeight: 90, borderRadius: 14, padding: 10, cursor: "pointer",
              display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center",
              fontSize: card.type === "term" ? 16 : 12, fontWeight: card.type === "term" ? 800 : 600,
              background: isUp ? (matched.includes(card.pair) ? "#22c55e" : accent) : t.surface,
              color: isUp ? "#fff" : t.muted, border: `2px solid ${isUp ? "transparent" : t.border}`,
              transition: "all .2s",
            }}>
              {isUp ? card.label : "?"}
            </div>
...
function Kahoot({ t, set, accent, onFinish }) {
  ...
  const COLORS = ["#0f766e", "#d97706", "#115e59", "#b45309"];
  const [i, setI] = useState(0);
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto", textAlign: "center" }}>
      <div style={{ display: "flex", justifyContent: "space-between", color: t.muted, fontWeight: 800 }}>
        ...
        <span style={{ color: time <= 5 ? "#ef4444" : t.text }}>⏱ {time}s</span>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {q.options.map((opt, idx) => {
          let bg = COLORS[idx];
          if (picked != null) bg = opt === q.correct ? "#22c55e" : (opt === picked ? "#94a3b8" : COLORS[idx] + "55");
          return (
            <button key={idx} onClick={() => picked == null && answer(opt)} style={{
              padding: 18, borderRadius: 14, border: "none", background: bg, color: "#fff",
...
function GoldHunt({ t, set, accent, onFinish }) {
  ...
  const answer = (opt) => {
    if (opt === q.correct) { setCoins((c) => c + 10); setRevealed((r) => [...r, i]); }
    if (i + 1 >= questions.length) {
      onFinish({ stars: stars(coins + (opt === q.correct ? 10 : 0), questions.length * 10 * 0.6), score: coins, total: questions.length * 10 });
    } else setI(i + 1);
  };

  return (
    <div style={{ maxWidth: 560, margin: "0 auto", textAlign: "center" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontWeight: 900, fontSize: 22, color: t.gold, marginBottom: 12 }}>
        <Star size={24} fill={t.gold} color={t.gold} /> {coins} coins
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 6, maxWidth: 280, margin: "0 auto 18px" }}>
        {questions.map((_, idx) => (
          <div key={idx} style={{
            ...
            background: revealed.includes(idx) ? t.gold : t.surface, border: `2px solid ${t.border}`,
          }}>{revealed.includes(idx) ? <Star size={18} fill="#fff" color="#fff" /> : <Lock size={14} color={t.muted} />}</div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "center", margin: "16px 0", flexWrap: "wrap" }}>
        {word.split("").map((l, idx) => (
          <div key={idx} style={{
            ...
            color: lost && !guessed.includes(l) ? "#ef4444" : t.text,
          }}>{guessed.includes(l) || lost ? l : ""}</div>
        ))}
      </div>
      <div style={{ color: t.muted, marginBottom: 10 }}>Lives: {parts - wrong}</div>
        ...
        {"ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map((l) => {
          ...
          return (
            <button key={l} onClick={() => guess(l)} disabled={used || ended} style={{
              ...
              background: used ? (ok ? "#22c55e" : "#94a3b8") : accent, color: "#fff", opacity: used ? 0.7 : 1,
            }}>{l}</button>
          );
        })}
      </div>
...
function Wordle({ t, set, accent, onFinish }) {
  // pick a word of consistent length; default to first item
  const word = useMemo(() => {
  ...
  const submit = () => {
    if (cur.length !== LEN || ended) return;
    setGuesses((g) => [...g, cur]);
    setCur("");
  };
  const type = (k) => {
  ...
  const letterState = (guess, idx) => {
    const l = guess[idx];
    if (word[idx] === l) return "#0f766e";      // correct spot — brand teal
    if (word.includes(l)) return "#d97706";     // in word, wrong spot — brand gold
    return "#64748b";                            // not in word — grey
  };
  const keyState = (l) => {
    let st = "";
    guesses.forEach((g) => g.split("").forEach((gl, idx) => {
      if (gl !== l) return;
      if (word[idx] === l) st = "#0f766e";
      else if (word.includes(l) && st !== "#0f766e") st = "#d97706";
      else if (!word.includes(l) && !st) st = "#64748b";
    }));
    return st;
  };
  ...
  return (
    <div style={{ maxWidth: 420, margin: "0 auto", textAlign: "center" }}>
        ...
        {rows.map((g, ri) => (
          <div key={ri} style={{ display: "flex", gap: 6 }}>
            {Array(LEN).fill(0).map((_, ci) => {
              ...
              const bg = isGuess ? letterState(g, ci) : "transparent";
              return (
                <div key={ci} style={{
                  width: 42, height: 42, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 22, fontWeight: 900, color: isGuess ? "#fff" : t.text,
                  background: bg, border: isGuess ? "none" : `2px solid ${t.border}`,
                }}>{filled ? (g[ci] || "") : ""}</div>
              );
            })}
          </div>
      ...
      <Keyboard t={t} accent={accent} onKey={type} keyState={keyState} />
    </div>
  );
}

function Keyboard({ t, accent, onKey, keyState }) {
  const rows = ["QWERTYUIOP", "ASDFGHJKL", "ZXCVBNM"];
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "center" }}>
      {rows.map((row, i) => (
        <div key={i} style={{ display: "flex", gap: 4 }}>
          {i === 2 && <KbKey label="ENTER" wide t={t} accent={accent} onKey={onKey} />}
          {row.split("").map((l) => <KbKey key={l} label={l} t={t} accent={accent} onKey={onKey} state={keyState(l)} />)}
          {i === 2 && <KbKey label="DEL" wide t={t} accent={accent} onKey={onKey} />}
        </div>
      ))}
    </div>
  );
}
function KbKey({ label, wide, t, accent, onKey, state }) {
  return (
    <button onClick={() => onKey(label)} style={{
      minWidth: wide ? 52 : 30, height: 46, borderRadius: 6, border: "none", cursor: "pointer",
      fontWeight: 800, fontSize: wide ? 11 : 14,
      background: state || t.surface, color: state ? "#fff" : t.text,
  ...
  return (
    <div style={{ padding: "28px 0", maxWidth: 640, margin: "0 auto" }}>
      <div style={{ ...card(t), textAlign: "center" }}>
        <div style={{ width: 80, height: 80, borderRadius: 999, background: t.brand, color: "#fff", fontSize: 34, fontWeight: 900, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 12 }}>
        {badges.map((b) => (
          <div key={b.name} style={{ ...card(t), textAlign: "center", opacity: b.earned ? 1 : 0.4 }}>
            <Trophy size={28} color={b.earned ? t.gold : t.muted} />
      ...
      <div style={card(t)}>
        {rows.map((r, i) => (
          <div key={i} style={{
            ...
            background: r.me ? t.brand + "14" : "transparent", borderRadius: 8,
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontWeight: 900, color: i < 3 ? t.gold : t.muted, width: 24 }}>#{i + 1}</span>
              <span style={{ fontWeight: 800, color: t.text }}>{r.name}{r.me && " (you)"}</span>
            </div>
            <span style={{ display: "flex", alignItems: "center", gap: 4, fontWeight: 800, color: t.gold }}>
              <Star size={15} fill={t.gold} color={t.gold} /> {r.stars}
            </span>
          </div>
        ))}
      </div>
...
function Admin({ t, sets, setSets }) {
  ...
  return (
    <div style={{ padding: "28px 0 60px" }}>
      ...
      <div style={{ display: "flex", gap: 8, margin: "18px 0" }}>
        {[["sets", "Sets", BookOpen], ["users", "Users", Users], ["codes", "Game codes", Hash]].map(([k, lbl, Ic]) => (
          <button key={k} onClick={() => setTab(k)} style={{
            ...
            border: "none", fontWeight: 800, background: tab === k ? t.brand : t.surface, color: tab === k ? "#fff" : t.text,
      ...
      {tab === "sets" && (
        <>
          <button onClick={() => setEditing({ id: "wk" + (sets.length + 1), week: sets.length + 1, title: "", topic: "Synonyms", description: "", published: false, releaseDate: "", items: [{ term: "", pos: "", def: "", example: "" }] })}
            style={{ ...pill(t.brand), display: "flex", alignItems: "center", gap: 6, marginBottom: 16 }}>
            ...
            {sets.map((s) => (
              <div key={s.id} style={{ ...card(t), display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => togglePublish(s.id)} style={{ ...pill(s.published ? "#94a3b8" : "#22c55e"), padding: "8px 14px", fontSize: 13 }}>
                    {s.published ? "Lock" : "Publish"}
                  </button>
                  <button onClick={() => setEditing(s)} style={iconBtn(t)}><Edit size={16} /></button>
                  <button onClick={() => remove(s.id)} style={{ ...iconBtn(t), color: "#ef4444" }}><Trash2 size={16} /></button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={card(t)}>
          {[{ n: "Maya R.", e: "maya@example.com", r: "student" }, { n: "Parent (Tom's mum)", e: "parent@example.com", r: "parent" }, { n: "Admin", e: "admin@wordscout11plus.com", r: "admin" }].map((u, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "12px 0", borderBottom: i < 2 ? `1px solid ${t.border}` : "none" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 800, color: t.brand, background: t.brand + "1a", padding: "4px 12px", borderRadius: 999, height: "fit-content" }}>{u.r}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ display: "grid", gap: 12 }}>
        ...
        {code && (
          <div style={{ textAlign: "center", padding: 20, background: t.bg, borderRadius: 12 }}>
            ...
            <div style={{ fontSize: 36, fontWeight: 900, letterSpacing: 6, color: t.brand }}>{code}</div>
          </div>
        )}
      </div>
    </div>
...
function SetEditor({ t, draft, onClose, onSave }) {
  ...
  return (
    <div style={{ position: "relative", minHeight: 400, background: "rgba(0,0,0,0.5)", borderRadius: 0, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "30px 0", marginTop: 20 }}>
      <div style={{ ...card(t), width: "100%", maxWidth: 640 }}>
        ...
        <div style={{ display: "grid", gap: 10, margin: "16px 0" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <input value={d.topic} onChange={(e) => upd("topic", e.target.value)} placeholder="Topic" style={selStyle(t)} />
            <input type="date" value={d.releaseDate} onChange={(e) => upd("releaseDate", e.target.value)} style={selStyle(t)} />
          </div>
        ...
        {d.items.map((it, i) => (
          <div key={i} style={{ border: `1px solid ${t.border}`, borderRadius: 12, padding: 12, marginBottom: 10 }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              <input value={it.term} onChange={(e) => updItem(i, "term", e.target.value)} placeholder="Term" style={{ ...selStyle(t), flex: 1 }} />
              <input value={it.pos} onChange={(e) => updItem(i, "pos", e.target.value)} placeholder="Part of speech" style={{ ...selStyle(t), flex: 1 }} />
              <button onClick={() => delItem(i)} style={{ ...iconBtn(t), color: "#ef4444" }}><Trash2 size={15} /></button>
            </div>
        ...
        <button onClick={addItem} style={{ ...iconBtn(t), width: "auto", padding: "8px 14px", gap: 6, display: "flex", marginBottom: 16 }}><Plus size={15} /> Add word</button>
        <div style={{ display: "flex", gap: 10 }}>
